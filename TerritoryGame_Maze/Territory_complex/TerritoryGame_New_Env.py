...
class TerritoryBattleEnvCoop(gym.Env):
    def __init__(self, grid_size=12, max_steps=150):
        super(TerritoryBattleEnvCoop, self).__init__()
        ...
        self.resource_point = (1, self.grid_size - 2)
        self.vision_range = 2
    ...
    def reset(self):
        ...
        # 固定中间九格为障碍物  Fix the middle nine grids as obstacles
        self._initialize_fixed_obstacles_and_resources()
    ...
    def _initialize_fixed_obstacles_and_resources(self):
   
        self.obstacle_positions.clear()
        self.resource_positions.clear()

        # 定义中间九格的障碍物位置# Define the obstacle positions in the middle nine grids
        ...
        # 在网格中标记障碍物Mark obstacles in the grid
        for position in self.obstacle_positions:
            self.grid[position] = 3  # 3表示障碍物

        # 初始化资源点Initialize resource points
        self.resource_positions.add(self.resource_point)
        self.grid[self.resource_point] = 4  # 4表示资源点 4 represents resource points
    ...
    def step(self, actions):
        ...
        # 检查障碍物并更新新位置# Check for obstacles and update new position
        for i, agent in enumerate(self.agents):
            if new_positions[i] in self.obstacle_positions:  # 如果新位置是障碍物
                new_positions[i] = old_positions[i]  # 保持原位
        ...
        for i, agent in enumerate(self.agents):
            ...
            if self.trajectories[new_positions[i]] == team_color:
                rewards[i] -= 0.1  # 如果移动到自己的色块，减少0.1奖励
        ...
        for i, agent in enumerate(self.agents):
            if agent['position'] in self.resource_positions:
                if agent['type'] == 'painter':
                    ...
                    self._color_around_resource(agent['position'], agent['team'])  # 将周围格子涂色# Color the surrounding grid
                else:
                    new_positions[i] = old_positions[i]  # If it is an attacking agent, it cannot eat resource points, it can only walk through them
        ...
        # 统计红队和蓝队的涂色格子数量Count the number of colored grids of the red team and the blue team
        ...
        for dx in [-1, 0, 1]:
            for dy in [-1, 0, 1]:
                if 0 <= x + dx < self.grid_size and 0 <= y + dy < self.grid_size:
                    if self.grid[x + dx, y + dy] != 3:  # 不涂色障碍物
